...
def render_geometric_lexicon():
    ...
    with st.expander(TXT("📜 مقدمة: لماذا المعجم الهندسي؟", "📜 Introduction: Why a Geometric Lexicon?"), expanded=True):
        st.markdown(TXT(
            ...
            <ul>
                ...
                <li>📜 <b>حكمة التاريخ:</b> في النظر إلى تكرار الحروف في فواتح السور، وربطها بأحداث الوجود الكبرى.</li>
                <li>🕋 <b>تواضع العارفين:</b> في الإقرار بأن هذا كله ليس إلا قطرة من بحر علم الله الذي لا قاع له، لقوله تعالى: ﴿وَمَا أُوتِيتُم مِّنَ الْعِلْمِ إِلَّا قَلِيلًا﴾.</li>
            </ul>
            </div>
            """,
            """
    ...
    with st.expander(TXT("١. الأساس: نظام أبجد هوز (حساب الجمل)", "1. Foundation: The Abjad Hawaz System"), expanded=False):
        ...
        abjad_table = {
            TXT("الآحاد", "Units"): {"أ": 1, "ب": 2, "ج": 3, "د": 4, "هـ": 5, "و": 6, "ز": 7, "ح": 8, "ط": 9},
            TXT("العشرات", "Tens"): {"ي": 10, "ك": 20, "ل": 30, "م": 40, "ن": 50, "س": 60, "ع": 70, "ف": 80, "ص": 90},
            TXT("المئات", "Hundreds"): {"ق": 100, "ر": 200, "ش": 300, "ت": 400, "ث": 500, "خ": 600, "ذ": 700, "ض": 800, "ظ": 900},
            TXT("الألف", "Thousand"): {"غ": 1000},
        }
        
        for group, letters in abjad_table.items():
            st.markdown(f"**{group}**")
        ...
        categories = {
            ...
            TXT("الفئة الثانية: حروف الازدواج (ثابت ومتجلي)", "Cat 2: Duality (Fixed & Manifested)"): {
                ...
                'desc': TXT(
                    ...
                    "ق (100) = الميزان الثابت / القسط المتجلي. ص (90) = الصمد الثابت / الصبر والصدق المتجلي.",
                    ...
                    "Q (100) = The fixed Balance / manifested Justice. S (90) = The fixed Eternal / manifested Patience and Truthfulness."
                )
            },
            TXT("الفئة الثالثة: حروف التجلي الإلهي", "Cat 3: Divine Manifestation"): {
                ...
                'desc': TXT(
                    ...
                    "أ=الوحدانية، ل=المُلك والعدل، م=الجمع والتماسك، ر=اليقظة والمراقبة، س=السمع والاستجابة، ح=الحياة والاستدامة، ط=الطهارة والمناعة.",
            ...
            TXT("الفئة الرابعة: حروف الاشتراك (الجسور)", "Cat 4: Connection (Bridges)"): {
                ...
                'desc': TXT(
                    ...
                    "المعادلات الوجودية: ك+ن=ع (الإدراك) ← ع+ي=80 (الاستجابة) ← ي+هـ=15 (الهوية) ← هـ→ك (العودة للمصدر).",
            ...
            TXT("الفئة الخامسة: المشغلات المفتاحية", "Cat 5: Key Operators"): {
                ...
                'desc': TXT(
                    "أربعة حروف تمثل أدوات المنطق والسببية في كلام الله. هي التي تحدد كيفية تفاعل الأسباب مع النتائج. فالفاء (=) للسببية، والواو (× أو +) للضرب أو الجمع، والباء (بـ) للاستعانة، واللام (→) للتعليل. "
                    "هذه المشغلات هي 'الأوامر البرمجية' (Operators) في نظام التشغيل القرآني.",
                    "Four letters representing the tools of logic and causality in Allah's speech. They determine how causes interact with results. Fa (=) for causality, Waw (× or +) for multiplication or addition, Ba (by) for assistance, Lam (→) for purpose. "
                    "These operators are the 'programming commands' in the Quranic operating system."
                )
            },
            TXT("الفئة السادسة: حروف أعمال الخلق", "Cat 6: Actions of Creation"): {
                ...
                'desc': TXT(
                    ...
                    "هذه الحروف هي 'المتغيرات التابعة' (Dependent Variables) في المعادلة، تتغير قيمتها بحسب اختيار الإنسان.",
                    ...
                    "These letters are the 'dependent variables' in the equation, their values changing according to human choice."
                )
            },
        }
