...
def plot_quadrant_map(B_raw, W_raw, istidraj_gap=0.0):
    ...
    ax.fill_between([-1.2, 0], 0, 1.2, color='#FF5252', alpha=0.2, label=TXT('الكافرون', 'Disbelievers'))
    ax.fill_between([-1.2, 0], -1.2, 0, color='#FFB6C1', alpha=0.2, label=TXT('المنافقون', 'Hypocrites'))
    ax.fill_between([0, 1.2], -1.2, 0, color='#FFA500', alpha=0.2, label=TXT('المشركون', 'Polytheists'))
