...
st.success("✅ V19 – الإعدادات ستظهر لك فوراً تحت هذا السطر")
...
st.markdown("---")
st.markdown("## 🎛️ جميع الخيارات التي تبحث عنها (اسحب للإسفل قليلاً إن احتجت)")

# جميع المتغيرات بشكل عمودي واضح (بدون أعمدة لتجنب مشاكل الجوال)
...
if st.session_state.get("run", False):
    ...
    # رسم صغير جداً حتى لا يخفي الإعدادات
    if st.session_state.init:
...
st.markdown("---")
st.markdown("<p style='text-align:center;color:gray;font-size:0.65em;'>© 2026 علي عادل العاطفي | V19</p>", unsafe_allow_html=True)
