...
T_texts = {
    "ar": {
        "title": "نظرية الميزان", "subtitle": "S = W × B | 28 حرفاً عربياً | الفيزياء والكيمياء والبيولوجيا والأخلاق",
        ...
        "welcome_title": "👋 مرحباً في مختبر الميزان!",
        "welcome_text": "الاستقرار الحقيقي (S) يأتي من الداخل (W × B)، وليس من التمكين الخارجي (E). عندما ترى قوماً ينهارون أخلاقياً ثم تُمطر عليهم النعم، فاعلم أنه استدراج – والنموذج يثبت ذلك.",
    ...
    "en": {
        "title": "The Mizan Theory", "subtitle": "S = W × B | 28 Arabic Letters | Physics, Chemistry, Biology & Ethics",
        "author": "Ali Adel Alatifi", "author_en": "",
        "run": "▶️ Run", "stop": "⏹️ Stop", "reset": "🔄 Reset", "download": "📥 Download",
        "footer": "Ali Adel Alatifi | 2026",
        "welcome_title": "👋 Welcome to the Mizan Lab!",
        "welcome_text": "True stability (S) comes from within (W × B), not from external empowerment (E).",
        "balance_good": "✅ System balanced",
        "simulation_stopped": "✅ Simulation stopped",
        "help_title": "📖 Guide", "help_content": "By Ali Adel Alatifi"
    }
}
T = T_texts.get(lang, T_texts["ar"])
...
st.markdown(f"""
<div style='text-align:center;background:linear-gradient(135deg,#1a1a2e,#16213e);padding:25px;border-radius:15px;border:2px solid #FFD700;margin-bottom:25px;'>
    <h1 style='color:#FFD700;font-size:36px;margin:0;'>⚖️ {T['title']} ⚖️</h1>
    <p style='color:#e0e0e0;font-size:16px;margin:15px 0 5px 0;'>{T['subtitle']}</p>
    <p style='color:#FFD700;font-size:20px;margin:15px 0 0 0;font-weight:bold;'>{T['author']}</p>
    <p style='color:#FFD700;font-size:14px;margin:5px 0 0 0;font-style:italic;opacity:0.8;'>{T['author_en']}</p>
</div>
...
with st.sidebar:
    ...
    st.markdown(f"<p style='text-align:center;color:#FFD700;font-size:14px;'>{T['author']} © 2026</p>", unsafe_allow_html=True)
    st.markdown("---")
...
# دوال الحساب والرسم (نفسها تماماً، لم تتغير للحفاظ على الوظائف)
def get_color(w, b):
    try:
        w=float(w); b=float(b)
...
st.markdown("<p style='text-align:center;color:#666;'>🧪 مختبر الميزان v5.0 – من الذرة إلى المجرة 🌌</p>", unsafe_allow_html=True)
