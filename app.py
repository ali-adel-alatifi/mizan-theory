...
<div style="background: linear-gradient(180deg, #000000 0%, #0a0a1a 30%, #1a1a2e 70%, #0a0a1a 100%); padding: 40px 20px; border-radius: 20px; margin-bottom: 30px; border: 2px solid #FFD700; box-shadow: 0 0 50px rgba(255, 215, 0, 0.4), 0 0 100px rgba(255, 215, 0, 0.1);">
    <div style="text-align: center; margin-bottom: 30px;">
        ...
        <p style="color: #aaaaaa; font-size: 16px; margin: 10px 0 0 0; letter-spacing: 2px;">{MAIN_SUBTITLE}</p>
    </div>
    ...
    <div style="text-align: center;">
        <p style="color: #FFD700; font-size: 20px; margin: 0; font-weight: bold;">{AUTHOR_AR}</p>
        <p style="color: #FFD700; font-size: 12px; margin: 5px 0 0 0; font-style: italic; opacity: 0.8;">{AUTHOR_EN}</p>
        <p style="color: #666; font-size: 12px; margin: 10px 0 0 0;">{FOOTER}</p>
    </div>
</div>
...
if st.session_state.get("run",False):
    ...
    while st.session_state.get("run",False):
        try:
            ...
            else: warn_placeholder.success("✅ توازن تام")
            time.sleep(0.08)
        except Exception as e:
            st.error(f"خطأ: {str(e)}")
...
if 'latest_image' in st.session_state:
    st.sidebar.download_button("📥 تحميل",st.session_state.latest_image,"mizan.png","image/png")

st.markdown("---")
st.caption("🧪 مختبر الميزان v6.0 – من الذرة إلى المجرة 🌌")
