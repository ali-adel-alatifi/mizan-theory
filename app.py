...
st.success("✅ V21 – هذا الكود مضمون ليعمل بشكل مستقر")
...
with c3:
    if st.button("🔄 إعادة ضبط", use_container_width=True):
        # إعادة ضبط آمنة بدون أوامر cache
...
# =========== جميع الخيارات التي تبحث عنها (ظاهرة مباشرة) ===========
st.markdown("---")
...
if not st.session_state.init:
    ...
    n = N_STARS if 'N_STARS' in locals() else 60
    st.session_state.cx=5; st.session_state.cy=5
...
if st.session_state.get("run", False):
    while st.session_state.get("run", False):
        try:
            ...
            ax.add_patch(Circle((5,5),2*S,color='#FFD700',alpha=0.2))
            ax.add_patch(Circle((5,5),1.5*E,color='#00FFFF',alpha=0.12))
            ax.text(5,5,'⚖️',fontsize=22,ha='center',va='center',color='#FFD700')
...
st.markdown("---")
st.markdown("<p style='text-align:center;color:gray;'>© 2026 علي عادل العاطفي</p>", unsafe_allow_html=True)
