...
# نظرية الميزان – الحروف العربية الـ 28
#
# © 2026 علي عادل العاطفي | Ali Adel Alatifi
# جميع الحقوق محفوظة.
# هذا البرنامج محمي بموجب رخصة GNU GPL v3.0.
# =============================================
...
with st.expander("📜 رسالة الترحيب – افتح للقراءة", expanded=False):
    ...
    with tab1:
        st.markdown("""
        <div style="text-align: center; line-height: 2; color: #CCCCCC;">
        > "هل يوجد قانون واحد يحكم الذرة والحضارة؟<br>
        > هذا هو نموذج الميزان الذي يثبت أن <b style="color: #FFD700;">S = W × B</b>"
        ---
        <b style="color: #FFD700;">الدين القيم</b> = قانون السببية الكوني.
    ...
    with tab2:
        ...
        - **⚪ أبيض/رمادي**: ولاء دون براءة.
        - **🔴 أحمر**: براءة دون ولاء.
        - **🩷 وردي/برتقالي**: خطر (كلاهما منخفض).
        - **الدائرة الذهبية**: قوة الاستقرار (S).
        - **القرص السماوي**: التمكين (E). إذا اتسع بينما يضيق الذهبي = استدراج.
        - **لوحة الإثبات**: الخط الذهبي (S) يقود الخط السماوي (E).
        """)
...
with st.sidebar:
    ...
    # --- إعدادات المحاكاة ---
    st.subheader("⚙️ إعدادات المحاكاة")
...
if st.session_state.get("run", False):
    ...
    if st.session_state.init:
        ...
        ax.set_title('اضغط ▶️ تشغيل في الشريط الجانبي', color='white', fontsize=14)
        st.pyplot(fig)
...
st.markdown("---")
st.markdown("<p style='text-align:center;color:gray;font-size:0.8em;'>© 2026 علي عادل العاطفي | جميع الحقوق محفوظة</p>", unsafe_allow_html=True)
