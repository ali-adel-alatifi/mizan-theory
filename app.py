...
st.set_page_config(page_title="نظرية الميزان – الورشة التفاعلية", page_icon="⚖️", layout="wide", initial_sidebar_state="expanded")
...
LANGUAGES = {
    "ar": {
        ...
        "author": "علي عادل العاطفي", "author_en": "Ali Adel Alatifi",
        ...
        "welcome": "### 👋 مرحباً في مختبر الميزان الشامل!\n\n> **الاستقرار الحقيقي (S) يأتي من الداخل (W × B)، وليس من التمكين الخارجي (E). عندما ترى قوماً ينهارون أخلاقياً ثم تُمطر عليهم النعم، فاعلم أنه استدراج.**\n\n---\n\n#### 🏆 الحقيقة المركزية\n\n| المستوى | كيف يظهر |\n|---------|----------|\n| ⚛️ الفيزياء | الإلكترون يدور باستقرار كلما زاد S |\n| 🧪 الكيمياء | الجزيء يهتز وينبض بقوة مع S |\n| 🧫 البيولوجيا | الغشاء والنواة ينبضان مع S |\n| 🧍 الأخلاق | النجوم تغير لونها حسب S = W × B |\n\n---\n\n> **الإسلام = الاستجابة للقانون الأعظم = توازن الولاء والبراءة = الثبات في كل شيء** 🌌\n\n---\n\n#### 📖 ﴿وَالسَّمَاءَ رَفَعَهَا وَوَضَعَ الْمِيزَانَ﴾\n\nاضغط **▶️ تشغيل** للبدء!",
    ...
    "en": {
        ...
        "footer": "Ali Adel Alatifi | 2026", "sidebar_author": "Ali Adel Alatifi © 2026",
        "welcome": "### 👋 Welcome to the Mizan Lab!\n\nPress **▶️ Run** to start!",
        "balance_good": "✅ System balanced", "simulation_stopped": "✅ Simulation stopped",
...
with col_lang:
    ...
    selected_lang = st.selectbox("اللغة / Language", options=list(lang_options.keys()), format_func=lambda x: lang_options[x], index=list(lang_options.keys()).index(lang), key="lang_selector")
    if selected_lang != lang: st.session_state.lang = selected_lang; st.rerun()

st.markdown(f"""<div style='text-align:center;background:linear-gradient(135deg,#1a1a2e,#16213e);padding:20px;border-radius:15px;border:2px solid #FFD700;margin-bottom:20px;'><h1 style='color:#FFD700;font-size:32px;margin:0;'>⚖️ {T['title']} ⚖️</h1><p style='color:#e0e0e0;font-size:16px;margin:10px 0 5px 0;'>{T['subtitle']}</p><p style='color:#FFD700;font-size:18px;margin:10px 0 0 0;font-weight:bold;'>{T['author']}</p><p style='color:#FFD700;font-size:14px;margin:5px 0 0 0;font-style:italic;opacity:0.8;'>{T.get('author_en','Ali Adel Alatifi')}</p></div>""", unsafe_allow_html=True)

with st.sidebar:
    st.markdown("""<div style='text-align:center;padding:10px;background:linear-gradient(135deg,#1a1a2e,#16213e);border-radius:10px;margin-bottom:5px;border:1px solid #FFD700'><h2 style='color:#FFD700;margin:0;'>⚖️ نظرية الميزان</h2><p style='color:#e0e0e0;font-size:12px;margin:5px 0;'>S = W × B</p></div>""", unsafe_allow_html=True)
    ...
    with st.expander("🔆 الحروف النورانية (14)", expanded=False):
        ...
        for letter, data in MIZAN_LETTERS["light"].items():
            light_vals[letter] = st.slider(f"{letter} ({data['label']}) – {data['value']}", 0.0, 1.0, 0.7, 0.01, help=data['aya'], key=f"l_{letter}")
    with st.expander("⚙️ الحروف المحايدة (6)", expanded=False):
        neutral_vals = {}
        for letter, data in MIZAN_LETTERS["neutral"].items():
            neutral_vals[letter] = st.slider(f"{letter} ({data['label']}) – {data['role']}", 0.0, 1.0, 0.5, 0.01, help=data['aya'], key=f"n_{letter}")
    with st.expander("🌑 الحروف الظلامية (8)", expanded=False):
        dark_vals = {}
        for letter, data in MIZAN_LETTERS["dark"].items():
            dark_vals[letter] = st.slider(f"{letter} ({data['label']}) – {data['value']}", 0.0, 1.0, 0.2, 0.01, help=data['aya'], key=f"d_{letter}")
    ...
    N_STARS = st.slider("عدد النجوم", 100, 600, 300, 50, key="ns")
...
st.markdown("<p style='text-align:center;color:#666;'>🧪 مختبر الميزان v5.0 – قانون واحد من الذرة إلى المجرة 🌌</p>", unsafe_allow_html=True)
