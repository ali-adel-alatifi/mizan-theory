...
def get_mizan_color(w, b):
    ...
    else: return '#FFF8DC' if w > b else '#FFA07A'
...
if st.session_state.run:
    ...
    while st.session_state.run:
        ...
        ax_cosmic.add_patch(Circle((cx,cy), 0.5+HALO_MAX*E, color='#00FFFF', alpha=0.25*(1-min(E,1))+0.04, zorder=7))
        ...
        for i in range(6):
            ...
            ax_cosmic.add_patch(Circle((cx+mr*np.cos(an), cy+mr*np.sin(an)), 0.4, color='#FFFFFF', alpha=0.3+0.5*avg_W, zorder=8))
        for i in range(6):
            an = np.pi - np.pi/4 + i*(np.pi/2)/5
            ax_cosmic.add_patch(Circle((cx+mr*np.cos(an), cy+mr*np.sin(an)), 0.4, color='#FF3333', alpha=0.25+0.35*avg_B, zorder=8))
...
if 'latest_image' in st.session_state:
    st.sidebar.download_button("📥 تحميل الصورة الحالية", st.session_state.latest_image, "mizan_civilization.png", "image/png")
