...
st.set_page_config(
    ...
    page_icon="⚖️",
    layout="wide",
    initial_sidebar_state="expanded"  # الشريط الجانبي مفتوح تلقائياً
)

# ====================== CSS للواجهة الكحلية الذهبية ======================
st.markdown("""
<style>
    /* خلفية متدرجة بدلاً من الأسود الصرف */
    .stApp {
        background: linear-gradient(180deg, #0a0a2e 0%, #0d0d1a 100%);
    }
    .big-title {
        font-size: 2.2em; font-weight: 900; color: #FFD700; text-align: center;
        margin: 10px 0 0 0; text-shadow: 0 0 15px rgba(255,215,0,0.4);
    }
    .sub-title {
        font-size: 1.1em; color: #CCCCCC; text-align: center; margin: 0 0 20px 0;
    }
    /* أزرار التحكم في الأسفل – صغيرة وأنيقة */
    .bottom-btn > button {
        border-radius: 8px; font-weight: bold; height: 2.4em; width: 100%;
        background: #1a1a3e; color: white; border: 1px solid #FFD700; font-size: 0.85em;
    }
    .bottom-btn > button:hover { background: #FFD700; color: black; }
    .metric-box {
    ...
    [data-testid="stSidebar"] { background: #05051a; }
    [data-testid="stSidebar"] h2, [data-testid="stSidebar"] h3 { color: #FFD700; }
...
with st.expander("📜 رسالة الترحيب – افتح للقراءة", expanded=False):
    ...
    ---
    <b>﴿فَأَقِمْ وَجْهَكَ لِلدِّينِ حَنِيفًا ۚ فِطْرَتَ اللَّهِ الَّتِي فَطَرَ النَّاسَ عَلَيْهَا﴾ — الروم 30</b>
    </div>
    """, unsafe_allow_html=True)
...
# ============================================================
#  🎛️ الشريط الجانبي – جميع الخيارات (مضمونة ولن تختفي)
# ============================================================
with st.sidebar:
    ...
    N_STARS = st.slider("عدد النجوم", 100, 600, 300, 50, key="nst")
...
if not st.session_state.init:
    ...
    cx, cy = 14, 10
    n = N_STARS if 'N_STARS' in locals() else 300
...
col1, col2, col3, col4 = st.columns(4)
with col1:
    if st.button("▶️ تشغيل", key="play_bottom", use_container_width=True):
        st.session_state.run = True
        st.rerun()
with col2:
    if st.button("⏹️ إيقاف", key="stop_bottom", use_container_width=True):
        st.session_state.run = False
        st.rerun()
with col3:
    if st.button("🔄 إعادة ضبط", key="reset_bottom", use_container_width=True):
        for k in list(st.session_state.keys()): del st.session_state[k]
        st.rerun()
with col4:
    if st.button("🧹 مسح التخزين المؤقت", key="clear_cache", use_container_width=True):
        st.cache_data.clear()
...
# تذييل
st.markdown("<p style='text-align:center;color:gray;font-size:0.7em;'>© 2026 علي عادل العاطفي | V28 – النسخة المعمارية</p>", unsafe_allow_html=True)
