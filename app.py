...
<style>
    ...
    .stButton > button:active { background: #FFD700; color: black; }
    .metric-box { background: rgba(255,255,255,0.05); border-radius: 10px; padding: 8px 2px; text-align: center; border: 1px solid rgba(255,215,0,0.2); margin: 2px 0; }
    .metric-val { font-size: 1.3em; font-weight: bold; margin: 0; }
    .metric-lbl { font-size: 0.6em; color: #aaa; margin: 0; }
    #MainMenu, footer, header {visibility: hidden;}
    .block-container { padding: 8px 5px 5px 5px; }
</style>
...
if 'init' in st.session_state and st.session_state.init:
    ...
    with m1:
        st.markdown(f'<div class="metric-box"><p class="metric-val" style="color:#FFD700;">{st.session_state.S:.2f}</p><p class="metric-lbl">⚖️ S</p></div>', unsafe_allow_html=True)
    with m2:
        st.markdown(f'<div class="metric-box"><p class="metric-val" style="color:#FFF;">{st.session_state.W:.2f}</p><p class="metric-lbl">🤍 W</p></div>', unsafe_allow_html=True)
    with m3:
        st.markdown(f'<div class="metric-box"><p class="metric-val" style="color:#FF5252;">{st.session_state.B:.2f}</p><p class="metric-lbl">❤️ B</p></div>', unsafe_allow_html=True)
    with m4:
        st.markdown(f'<div class="metric-box"><p class="metric-val" style="color:#00FFFF;">{st.session_state.E:.2f}</p><p class="metric-lbl">💫 E</p></div>', unsafe_allow_html=True)

# ===================== جميع المتغيرات (مفتوحة افتراضياً) =====================
# استخدام st.session_state للتحكم في ظهور/اختفاء كل الإعدادات
if 'show_all_settings' not in st.session_state:
    st.session_state.show_all_settings = False

# زر واحد لفتح/إغلاق كل الإعدادات المتقدمة
if st.button("🔽 إظهار/إخفاء كل الإعدادات المتقدمة", use_container_width=True):
    st.session_state.show_all_settings = not st.session_state.show_all_settings
    st.rerun()

# حالة الزر الحالية
show = st.session_state.show_all_settings

# تعريف كل المتغيرات بقيم افتراضية (حتى لو كانت مخفية)
# هذا يمنع أخطاء "المتغير غير معرف"
...
riba = 0.2; ghish = 0.2; kadhib = 0.2
cycle_speed = 0.12; delay_frames = 22; N_STARS = 60

if show:
    st.markdown("---")
    st.markdown("### 🎛️ جميع الإعدادات المتقدمة")
    ...
    st.info("💡 اضغط على الزر أعلاه 🔽 لإظهار الإعدادات المتقدمة")
...
def calculate_S_full(W, B, E, prayer, zakat, fasting, hajj,
    ...
    S_final *= (0.5 + 0.5 * pillars_boost) * (0.8 + 0.4 * protection) * (0.9 + 0.2 * adl) * (0.85 + 0.3 * shura)
...
if not st.session_state.init:
    ...
    st.session_state.sx = np.random.uniform(0, 10, n)
    st.session_state.sy = np.random.uniform(0, 10, n)
    ...
    st.session_state.pS = deque(maxlen=150); st.session_state.pE = deque(maxlen=150)
    st.session_state.px = deque(maxlen=150)
...
if st.session_state.get("run", False):
    while st.session_state.get("run", False):
        try:
            ...
            for i in range(n):
                ...
                sw[i] += prayer*0.01 + hajj*0.005
                sb[i] += fasting*0.01 + zakat*0.005
            ...
            ins = 1 - np.mean(sw * sb)
            sx += np.random.uniform(-0.06,0.06,n)*ins
            sy += np.random.uniform(-0.06,0.06,n)*ins
            ...
            ax.set_title(f'{ph}  |  S={S:.2f}  |  E={E:.2f}', color='white', fontsize=12, fontweight='bold')

            ax2 = ax.inset_axes([0.08, -0.22, 0.84, 0.22])
            ...
            ax2.set_ylim(0,1.05); ax2.tick_params(colors='white', labelsize=6); ax2.grid(True, alpha=0.15)
    ...
    fig, ax = plt.subplots(figsize=(5,3.5), facecolor='#0a0a1a')
    ax.set_facecolor('#0a0a1a')
    colors = [get_mizan_color(st.session_state.sw[i], st.session_state.sb[i]) for i in range(len(st.session_state.sw))]
    ax.scatter(st.session_state.sx, st.session_state.sy, c=colors, s=45, alpha=0.9, edgecolors='white', linewidth=0.3)
    ax.add_patch(Circle((5,5), 2*st.session_state.S, color='#FFD700', alpha=0.2))
    ax.text(5,5,'⚖️',fontsize=28,ha='center',va='center',color='#FFD700')
    ax.set_xlim(0,10); ax.set_ylim(0,10); ax.axis('off')
    ax.set_title('اضغط ▶️ تشغيل للبدء', color='white', fontsize=14)
    plot_placeholder.pyplot(fig)
    plt.close(fig)

st.markdown("---")
st.markdown("<p style='text-align:center;color:gray;font-size:0.65em;'>© 2026 علي عادل العاطفي | النسخة النهائية</p>", unsafe_allow_html=True)
