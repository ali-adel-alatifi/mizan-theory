...
# =============================================
# 🎨 CSS – خلفية كحلية متدرجة (وليس أسود)
# =============================================
st.markdown("""
<style>
...
# 🏛️ العنوان
# =============================================
st.title("⚖️ الدِّينُ الْقَيِّم ⚖️" if L == "ar" else "⚖️ Al-Deen Al-Qayyim ⚖️")
st.header("قَانُونُ التَّوَازُنِ الْكَوْنِيّ" if L == "ar" else "The Cosmic Balance Law")
st.caption("S = W × B | © 2026 Ali Adel Alatifi | علي عادل العاطفي")

# =============================================
# 📜 رسالة الترحيب (مبسطة وواضحة)
# =============================================
with st.expander("📜 رسالة الترحيب" if L == "ar" else "📜 Welcome", expanded=False):
    if L == "ar":
        st.markdown("""
        **S = W × B**
        ...
        **الألوان:** ذهبي = توازن | أبيض = ولاء | أحمر = براءة | وردي = خطر
        **العناصر:** الدائرة الذهبية = S | القرص السماوي = E | لوحة الإثبات = S يقود E
        """)
    else:
        st.markdown("""
        **S = W × B**
        ...
        It is the Balance upon which heavens and earth were established.

        **Colors:** Gold = Balance | White = Allegiance | Red = Disavowal | Pink = Danger
        **Elements:** Golden circle = S | Cyan disc = E | Evidence panel = S leads E
        """)

# =============================================
# 📖 كتاب الميزان (محتوى كامل)
# =============================================
with st.expander("📖 كتاب الميزان" if L == "ar" else "📖 The Mizan Book", expanded=False):
    st.markdown("""
    **نظرية الميزان – من الثنائية الكونية إلى معادلة الوجود S = W × B**

    ---

    **الباب الأول: الأصول** – ﴿اقْرَأْ بِاسْمِ رَبِّكَ﴾ المنهج الإلهي لاكتشاف النظام.

    **الباب الثاني: التعريفات المركزية** – الدين القيم، الإسلام الحنيف، العقيدة، العبادة، الشريعة.

    **الباب الثالث: الثنائية الكونية** – الولاء والبراء في مرآة الوجود، التطابق مع الفيزياء والبيولوجيا.

    **الباب الرابع: وحدة الخلق والأمر** – ﴿أَلَا لَهُ الْخَلْقُ وَالْأَمْرُ﴾ الميزان في الآفاق والأنفس.

    **الباب الخامس: الدورة الإلهية المحكمة** – من الأزل إلى الخلود، مراحل الوجود الخمس.

    **الباب السادس: المختبر** – النمذجة الحاسوبية، البرهان العملي على S = W × B وطبقة الآخرة.

    **الباب السابع: الفطرة** – البوصلة الأصيلة، الميل الفطري للإيمان.

    **الباب الثامن: سبل العودة إلى القانون** – الإصلاح الفردي والأسري والمجتمعي.

    **الباب التاسع: التشخيص** – واقع الأمة في ميزان S = W × B.
...
# =============================================
with st.sidebar:
    st.header("🎛️ لوحة التحكم" if L == "ar" else "🎛️ Control Panel")

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        if st.button("▶️" if L == "en" else "▶️", use_container_width=True): st.session_state.run = True
    with c2:
        if st.button("⏹️" if L == "en" else "⏹️", use_container_width=True): st.session_state.run = False
    with c3:
        if st.button("🔄" if L == "en" else "🔄", use_container_width=True):
    ...
    with st.expander("🕌 أركان الإسلام" if L == "ar" else "🕌 Pillars", expanded=False):
        prayer = st.slider("الصلاة" if L == "ar" else "Prayer", 0.0, 1.0, 0.8, 0.01, key="p")
        zakat = st.slider("الزكاة" if L == "ar" else "Zakat", 0.0, 1.0, 0.6, 0.01, key="z")
        fasting = st.slider("الصوم" if L == "ar" else "Fasting", 0.0, 1.0, 0.7, 0.01, key="f")
        hajj = st.slider("الحج" if L == "ar" else "Hajj", 0.0, 1.0, 0.5, 0.01, key="h")

    with st.expander("🏛️ أسس الحكم" if L == "ar" else "🏛️ Governance", expanded=False):
        amr = st.slider("الأمر بالمعروف" if L == "ar" else "Enjoining Good", 0.0, 1.0, 0.5, 0.01, key="amr")
        nahy = st.slider("النهي عن المنكر" if L == "ar" else "Forbidding Evil", 0.0, 1.0, 0.5, 0.01, key="nahy")
        adl = st.slider("العدل" if L == "ar" else "Justice", 0.0, 1.0, 0.6, 0.01, key="adl")
        shura = st.slider("الشورى" if L == "ar" else "Consultation", 0.0, 1.0, 0.5, 0.01, key="shura")

    with st.expander("🛡️ آليات الإصلاح" if L == "ar" else "🛡️ Reform", expanded=False):
        taawun_birr = st.slider("التعاون على البر" if L == "ar" else "Coop. in Righteousness", 0.0, 1.0, 0.5, 0.01, key="tb")
        tawasi_haqq = st.slider("التواصي بالحق" if L == "ar" else "Enjoining Truth", 0.0, 1.0, 0.5, 0.01, key="th")
        taawun_taqwa = st.slider("التعاون على التقوى" if L == "ar" else "Coop. in Piety", 0.0, 1.0, 0.5, 0.01, key="tt")
        tawasi_sabr = st.slider("التواصي بالصبر" if L == "ar" else "Enjoining Patience", 0.0, 1.0, 0.5, 0.01, key="ts")

    with st.expander("💀 آليات الإفساد" if L == "ar" else "💀 Corruption", expanded=False):
        nahy_marouf_e = st.slider("النهي عن المعروف" if L == "ar" else "Forbidding Good", 0.0, 1.0, 0.2, 0.01, key="nm")
        amr_munkar_e = st.slider("الأمر بالمنكر" if L == "ar" else "Enjoining Evil", 0.0, 1.0, 0.2, 0.01, key="amr_e")
        taawun_ithm = st.slider("التعاون على الإثم" if L == "ar" else "Coop. in Sin", 0.0, 1.0, 0.2, 0.01, key="ti")
        taawun_udwan = st.slider("التعاون على العدوان" if L == "ar" else "Coop. in Aggression", 0.0, 1.0, 0.2, 0.01, key="tu")
        tawasi_batil = st.slider("التواصي بالباطل" if L == "ar" else "Enjoining Falsehood", 0.0, 1.0, 0.2, 0.01, key="tbat")
        adam_sabr = st.slider("عدم الصبر" if L == "ar" else "Impatience", 0.0, 1.0, 0.2, 0.01, key="as")

    with st.expander("⚠️ الأمراض الأخلاقية" if L == "ar" else "⚠️ Moral Diseases", expanded=False):
        riba = st.slider("الربا" if L == "ar" else "Riba", 0.0, 1.0, 0.2, 0.01, key="riba")
        ghish = st.slider("الغش" if L == "ar" else "Deception", 0.0, 1.0, 0.2, 0.01, key="ghish")
        kadhib = st.slider("الكذب" if L == "ar" else "Lying", 0.0, 1.0, 0.2, 0.01, key="kadhib")

    with st.expander("⚙️ إعدادات المحاكاة" if L == "ar" else "⚙️ Settings", expanded=True):
        cycle_speed = st.slider("سرعة الدورة" if L == "ar" else "Cycle Speed", 0.001, 0.05, 0.008, 0.001, key="spd")
        delay_frames = st.slider("تأخير التمكين" if L == "ar" else "Empowerment Delay", 5, 50, 22, 1, key="dly")
        N_STARS = st.slider("عدد النجوم" if L == "ar" else "Number of Stars", 50, 300, 150, 10, key="nst")
...
if st.session_state.get("run", False):
    while st.session_state.run:
        try:
            ...
            break
    st.success("⏸️ تم إيقاف المحاكاة" if L == "ar" else "⏸️ Simulation stopped.")
else:
    if st.session_state.init:
...
# =============================================
# 📥 زر التحميل (ظاهر أسفل الصفحة)
# =============================================
if 'latest_image' in st.session_state:
    st.download_button(
        "📥 تحميل صورة المشهد" if L == "ar" else "📥 Download Scene Image",
