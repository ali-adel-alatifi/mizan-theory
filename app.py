...
<style>
    ...
    .big-title { font-size: 2.2em; font-weight: 900; color: #FFD700; text-align: center; margin: 10px 0 0 0; text-shadow: 0 0 15px rgba(255,215,0,0.4); }
    .sub-title { font-size: 1.1em; color: #CCCCCC; text-align: center; margin: 0 0 20px 0; }
    .stButton > button { border-radius: 12px; font-weight: bold; height: 3em; border: 2px solid #FFD700; background: #1a1a2e; color: white; }
    .stButton > button:hover { background: #FFD700; color: black; }
    .metric-box { background: rgba(255,255,255,0.05); border-radius: 10px; padding: 10px 5px; text-align: center; border: 1px solid rgba(255,215,0,0.2); }
    .metric-val { font-size: 1.4em; font-weight: bold; margin: 0; }
    .metric-lbl { font-size: 0.7em; color: #aaa; margin: 0; }
    [data-testid="stExpander"] details { background: #050515; border: 1px solid #FFD700; border-radius: 10px; margin-bottom: 8px; }
    [data-testid="stExpander"] summary { color: #FFD700; font-weight: bold; font-size: 1.1em; }
...
# ====================== جميع الخيارات في أقسام قابلة للطي ======================
st.markdown("---")
st.markdown("## 🎛️ لوحة التحكم الكونية")
...
# 2. أسس الحكم
with st.expander("🏛️ أسس الحكم", expanded=True):
...
# 3. آليات الإصلاح
with st.expander("🛡️ آليات الإصلاح", expanded=True):
...
# 4. آليات الإفساد
with st.expander("💀 آليات الإفساد", expanded=True):
...
# 5. الأمراض الأخلاقية
with st.expander("⚠️ الأمراض الأخلاقية", expanded=True):
...
with st.expander("⚙️ إعدادات المحاكاة", expanded=True):
    cycle_speed = st.slider("سرعة الدورة", 0.001, 0.05, 0.01, 0.001, key="spd")
...
# ====================== منطقة الرسم (لن تؤثر على الموسعات) ======================
...
st.markdown("---")
st.markdown("<p style='text-align:center;color:gray;'>© 2026 علي عادل العاطفي | V27 – النسخة الإمبراطورية</p>", unsafe_allow_html=True)
