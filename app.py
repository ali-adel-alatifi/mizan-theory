...
<style>
    ...
    .big-title { font-size: 2em; font-weight: 900; color: #FFD700; text-align: center; margin: 10px 0 0 0; }
    .sub-title { font-size: 1em; color: #CCCCCC; text-align: center; margin: 0 0 15px 0; }
    .stButton > button { border-radius: 8px; font-weight: bold; height: 2.6em; border: 1px solid #FFD700; background: #1a1a3e; color: white; }
    .stButton > button:hover { background: #FFD700; color: black; }
    [data-testid="stExpander"] details { background: #0d0d2b; border: 1px solid #FFD700; border-radius: 8px; margin-bottom: 6px; }
...
# ====================== أزرار التحكم (صف واحد) ======================
col_play, col_stop, col_reset, col_cache = st.columns(4)
with col_play:
    ...
    while st.session_state.run:
        try:
            ...
            if pSl:
                ...
                pax.set_title('📈 لوحة الإثبات: S يقود E', color='white', fontsize=8)
...
st.markdown("---")
st.markdown("<p style='text-align:center;color:gray;'>© 2026 علي عادل العاطفي | V29</p>", unsafe_allow_html=True)
