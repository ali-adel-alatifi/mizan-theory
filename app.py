...
# =============================================
# 📜 رسالة الترحيب – محتوى محسّن وواضح
# =============================================
with st.expander("📜 رسالة الترحيب – افتح للقراءة", expanded=False):
    ...
    with tab1:
        ...
        """, unsafe_allow_html=True)
    with tab2:
        # --- تم إصلاح النص بالكامل لجعله مقروءًا ---
        st.markdown("""
        ### 🔍 دليل قراءة المشهد
        
        **الألوان والمعاني:**
        - **ذهبي**: توازن مثالي (الولاء W والبراءة B مرتفعان).
        - **أبيض / رمادي**: غلبة الولاء (W) مع ضعف البراءة (B).
        - **أحمر**: غلبة البراءة (B) مع ضعف الولاء (W).
        - **وردي / برتقالي**: منطقة الخطر (الولاء والبراءة منخفضان).
        
        **عناصر المشهد:**
        - **الدائرة الذهبية حول المركز**: قوة الاستقرار (S).
        - **القرص السماوي**: التمكين (E). إذا اتسع القرص السماوي بينما تضيق الدائرة الذهبية، فهذا هو **الاستدراج**.
        - **الميزان في الأعلى**: يوضح نسبة W (أبيض) إلى B (أحمر).
        - **لوحة الإثبات أسفل الرسم**: الخط الذهبي (S) يقود الخط السماوي (E). في الاستدراج، ينخفض الخط الذهبي أولاً ثم يتبعه الخط السماوي بعد فترة.
        """)

st.markdown("---")

# =============================================
# 🎛️ الشريط الجانبي – كل المتغيرات (نفس الكود المستقر)
# =============================================
with st.sidebar:
...
if st.session_state.get("run", False):
    while st.session_state.run:
        try:
            ...
            # ---------- المشهد الكوني مع لوحة إثبات مكبرة ----------
            ...
            ax.add_patch(Circle((cx,cy), 0.5+14*E, color='#00FFFF', alpha=0.2, zorder=7))
            ...
            ax.text(14, 1.2, f'{ph} | S={S:.2f} | E={E:.2f}', color='white', fontsize=16,
            ...
            # حفظ الصورة للتحميل
            buf = BytesIO()
            fig.savefig(buf, format='png', dpi=120, facecolor='#000010')
            buf.seek(0)
...
# =============================================
# 📥 زر تحميل الصورة – ظاهر بوضوح أسفل الصفحة
# =============================================
st.markdown("---")
if 'latest_image' in st.session_state:
    with st.container():
        ...
        with col_dl:
            st.download_button(
                label="📥 تحميل صورة المشهد الحالية",
                data=st.session_state.latest_image,
                file_name="الميزان_الكوني.png",
                mime="image/png",
                use_container_width=True
            )
        with col_sp:
            st.markdown("☝️ اضغط هنا لتنزيل آخر حالة من المحاكاة")
else:
    st.info("سيظهر زر تحميل الصورة هنا بعد تشغيل المحاكاة لأول مرة.")

st.markdown("<p style='text-align:center;color:gray;font-size:0.8em;'>© 2026 Ali Adel Alatifi | Al-Deen Al-Qayyim – The Cosmic Balance Law</p>", unsafe_allow_html=True)
